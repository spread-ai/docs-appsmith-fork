---
Title: List
Description: List widget reference
---
...
Has an array of objects where each object represents a widget within the list items and holds information about the widgets' state.
...
Has an array of objects that each represents a list item and its data.
...
Has a number representing which page of the List is currently displayed.
...
Has a number representing the number of list items that can fit on one page of the List.
...
Has an object representing the state of the widgets inside a list item when it's selected.
...
Has an object representing the data of the list item that's selected when you click the list item card or when you interact with an widget (such as a button) inside the list item.
...
Has an object representing the state of the widgets inside a list item that's selected when you click the list item card or when you interact with a widget (such as a button) inside the list item.
...
1. Select the Image widget and add  `{{ '{{currentItem.bookImage}}' }}` under **Image** property. **currentItem** refers to the data for a particular item. All the list items get populated based on the corresponding data in the JSON object.
2. You can now see the image in the list item, as the image widget renders the image available on the URL supplied in JSON.
...
In the preceding example, if you want to display the book name of the selected item in a Text widget, bind it in the **Text** property of the Text widget as shown below
...
If you have an Input widget and a Button widget inside the List and want to use the Input's **Text** property to show an alert message on button click. In the button widget's *onClick* event, you can access the input widget's value as shown below
...
* The **currentItemsView** helps you to view the data and the state of the widgets present in all the items of the List widget and is an array of objects with each widget's state represented as an object. The array of objects is limited to the number of items visible on the page rather than the number list items present. If there are 300 objects in the list data, but the List widget is showing 5 items per page, then the **currentItemsView** property shows an array of only five objects.

```js title="Example of the evaluated value for all the items of the List"
[
     {
          "Image1": {
