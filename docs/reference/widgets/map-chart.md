---
Title: Map Chart
Description: Map Chart widget reference
---
...
Additionally, you can display dynamic data from queries or JavaScript functions by binding the response to the **Chart data** property. For example, if you have a query named `fetchData`, you can bind its response using:
...
Sets the action - framework functions, queries, or JavaScript functions - to be executed when a user clicks on a data point in the chart.
...
```js
{{ '{{MapChart1.selectedDataPoint}}' }} // (1)!
{{ '{{MapChart1.selectedDataPoint.id}}' }} // (2)!
{{ '{{MapChart1.selectedDataPoint.value}}' }} // (3)!
```
